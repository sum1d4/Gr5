...
// 2. データの取得とバリデーション
// target_age, subject, category はご要望の固定値として設定
$target_age = 1;        // ★ 1年生
$subject = 'tasi';      // ★ 科目を「たし算 (tasi)」に設定
$category = 'score';    // ★ カテゴリを「スコア (score)」に設定
...
if ($user_id === false || $user_id === null || $user_id <= 0) {
    // 認証されていないユーザーとみなし、401 Unauthorizedを返す
...
try {
    ...
    // 成功応答
    echo json_encode(['success' => true, 'message' => 'Score registered successfully (Tashizan 1D).']);
